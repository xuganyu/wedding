...
<link href="<?php echo base_url('zeros/web/css/select2.min.css'); ?>" rel="stylesheet" type="text/css" />
<script type="text/javascript" src="<?php echo base_url('zeros/web/js/jquery-2.1.4.min.js'); ?>"></script>
<script type="text/javascript" src="<?php echo base_url('zeros/web/js/jquery-select2/select2.min.js'); ?>"></script>
...
$(function(){
	...
	var province_str = '', province_sel;
	$.each(china_city, function(ck, cv){
		province_str += '<option data-name="' + cv.n + '">' + cv.n + '</option>';
	});
	$('#shengfen').append(province_str);	

	$('#shengfen').change(function(){
		var city_str = '<option value="0">城市</option>';
		$.each(china_city, function(ck, cv){
		    if($('#shengfen').val() == cv.n){
			    province_sel = cv;
			    if(cv.hasOwnProperty('d')){
				    $('#xiaqu').select2('enable', false);
			    }else{
				    $('#xiaqu').select2('enable', true);
			    }
		...
		$('#chengshi').html(city_str);
	});	

	$('#chengshi').change(function(){
	    var country_str = '<option value="0">辖区</option>';
	    $.each(province_sel, function(pk, pv){
	        if($('#chengshi').val() == pv.n && !province_sel.hasOwnProperty('d')){
	            $.each(pv, function(k, v){
		            if(k != 'n'){
    	                country_str += '<option value="' + v.n + '">' + v.n + '</option>';
		            }
	            });
	        }
	    });
	    $('#xiaqu').html(country_str);
	});
})
...
<div class="top" style="text-align: center;">
    <div style="width: 985px;height: 134px;margin: 0 auto;text-align: right;">
        ...
        <div class="pf-user">
            ...
            <h4 class="pf-user-name ellipsis" style="line-height: 70px;font-weight: bold;margin-top: 0;"><?php echo $login['user_nickname']; ?></h4>
                        ...
                        <LI style="margin-bottom: 20px;">
                            <div class="boxgrid captionfull" style="border: 1px solid #ddd;">
                                <a href="<?php echo site_url('web/search/info/'.$item['Id']); ?>" target="_blank">
                                    <img src="<?php echo base_url('uploads/user/'.$item['photo']); ?>" width="100%" height="100%"/>
                                </a>
