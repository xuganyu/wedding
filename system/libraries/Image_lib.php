...
*/
	//修改了image_lib扩展类，传参发生变化
	public function __construct($props = array()){
		if(count($props) > 0){
			$width = isset($props['width'])?$props['width']:75;
			$height = isset($props['height'])?$props['height']:50;
			$zongheng = isset($props['maintain_ratio'])?$props['maintain_ratio']:FALSE;
			
			$imgconfig = array();
			$imgconfig['image_library'] = 'gd2';
			$imgconfig['source_image'] = './uploads/'.$props['image'];
			...
			$imgconfig['maintain_ratio'] =$zongheng;   //设置为 FALSE  不保持纵横比， TRUE 保持纵横比
		}
		if(isset($imgconfig)){
			$this->initialize($imgconfig);	
		}
		log_message('debug', "Image Lib Class Initialized");	
	}
